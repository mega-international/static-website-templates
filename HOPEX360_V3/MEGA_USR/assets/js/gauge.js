(function () {
	...
	GaugeChartHelper.prototype.applyGaugeConfig = function(chartConfig) {
		...
		this.calcLimits(chartConfig.data.datasets[0].showLabels);
	...
	GaugeChartHelper.prototype.calcLimits = function(showLabelsBoolean) {
	...
	GaugeChartHelper.prototype.renderValueLabel = function() {
		...
		var newFontSize = this.data.fontSize ? this.data.fontSize : Math.floor(30 * widthRatio);
		var fontSizeToUse = Math.min(newFontSize, this.gaugeRadius);
		ctx.textAlign = "center";
		ctx.font = fontSizeToUse + "vh " + this.fontStyle;
	...
	GaugeChartHelper.prototype.renderArrow = function(radius, angle, arrowLength, arrowAngle, arrowColor) {
		...
		ctx.lineTo(this.gaugeCenterX, this.gaugeCenterY - (document.documentElement.clientHeight / 100 * this.data.fontSize));
		ctx.stroke();
